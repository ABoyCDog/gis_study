...
var Water = function (geometry, options) {
	...
	/**
	 * 
	 * @param {WebGLRenderer|THREE.WebGLRenderer} renderer 
	 * @param {THREE.WebGLRenderTarget|THREE.WebGLRenderTargetCube} renderTarget 
	 * @param {number} [activeCubeFaceIndex] 
	 * @param {string|HTMLCanvasElement} [writeTargetCanvas] 
	 */
	function readRenderTarget(renderer, renderTarget, activeCubeFaceIndex, writeTargetCanvas) {

		if (typeof writeTargetCanvas == 'string') writeTargetCanvas = document.getElementById(writeTargetCanvas);
		if (!writeTargetCanvas) writeTargetCanvas = document.createElement('canvas');

		let format = renderTarget.texture.format

		var size = 4;
		if (format == RGBFormat) {
			size = 3;
		}
		let pixels = new Uint8Array(renderTarget.width * renderTarget.height * size)
		renderer.readRenderTargetPixels(renderTarget, 0, 0, renderTarget.width, renderTarget.height, pixels, activeCubeFaceIndex);

		let cv = writeTargetCanvas;
		cv.width = renderTarget.width;
		cv.height = renderTarget.height;
		...
		if (size == 4) {
			...
			for (let i = 0, j = 0; i < imgData.data.length; i += 4, j += size) {

				for (let k = 0; k < size; k++) {
					imgData.data[i + k] = pixels[j + k];
				}
		...
		return writeTargetCanvas;
	}

	var cv1 = document.getElementById('cv-1')
	var cv2 = document.getElementById('cv-2')
